...
tp <- 10
(title <- paste(data.votacoes[tp,"proposicao"],data.votacoes[tp,"descricao"],data.votacoes[tp,"data"]))
cat(title,file="title.txt")
cat(with(data.votacoes[tp,],paste('images/',datavot,'.',numvot,'.png',sep='')),file="imagelink.txt")
...
data.votacoes$descricao <- with(data.votacoes,
paste('<a href="http://www.camara.gov.br/sileg/Prop_Lista.asp?Sigla=',
tipo,
                                 '&Numero=',
                                 numero,
                                 '&Ano=',ano,'">',descricao,'</a>',sep=''))
data.votacoes <- subset(data.votacoes,select=c(data,proposicao,descricao,resultado,mapa))
post <- paste("Link para a proposicao na Camara: ",data.votacoes[tp,"descricao"],data.votacoes[tp,"mapa"],data.votacoes[tp,"resultado"])
cat(post,file="post.txt")
post.id <- system("python post.py",intern=TRUE)
