...
for (nvotnow in data.votacoes$numvot) {
  ...
  col.vec <- c(rev(brewer.pal(n1,"Blues")[-1]),"grey",brewer.pal(n1,"Reds")[-1])
  ...
  mtext(wordwrap(paste(dn$texordia),40),3, cex=.9)
  dev.off()
  ...
  p <- p+opts(title=wordwrap(subset(data.votacoes,numvot==nvotnow)$texordia,40))
